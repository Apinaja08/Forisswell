...
function EventsPage() {
  ...
  const mapContainerId = "event-map";
  ...
  const geocodeAddress = async (address, city) => {
    ...
    try {
      const response = await fetch(`${url}?q=${encodeURIComponent(fullAddress)}&format=json&limit=1`, {
        headers: {
  ...
  const handleAddressSearch = async () => {
    ...
    try {
      ...
      if (coords) {
        ...
        setFormSuccess("Location coordinates found!");
        setTimeout(() => setFormSuccess(""), 3000);
      } else {
        setFormError("Could not find coordinates for this address");
      }
    } catch (err) {
      setFormError("Error finding location");
    }
  };
  ...
  const handleSubmitEvent = async (e) => {
    ...
    try {
      ...
      if (formData.location.address && formData.location.city && 
        ...
        try {
          const coords = await geocodeAddress(formData.location.address, formData.location.city);
          if (coords) {
            formData.location.coordinates = coords;
          }
        } catch (err) {
          console.warn("Geocoding failed, saving without coordinates");
        }
      }
  ...
  const isUserJoined = (event) => {
    ...
    const currentUserId = normalizeId(userId);
    return event.participants.some((p) => normalizeId(p.user) === currentUserId);
  };

  const getUserParticipantStatus = (event) => {
    if (!event?.participants || !userId) return null;
    const currentUserId = normalizeId(userId);
    const participant = event.participants.find((p) => normalizeId(p.user) === currentUserId);
  ...
  const MapModal = () => {
    useEffect(() => {
      if (showMapModal && selectedEvent?.location?.coordinates?.lat && selectedEvent?.location?.coordinates?.lng) {
        // Small delay to ensure DOM is ready
        const timer = setTimeout(() => {
          initMap(
            parseFloat(selectedEvent.location.coordinates.lat),
            parseFloat(selectedEvent.location.coordinates.lng),
      ...
      <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
        <div className="bg-white rounded-lg max-w-4xl w-full">
          <div className="p-4 border-b flex justify-between items-center">
            <h3 className="text-xl font-bold text-slate-900">Event Location</h3>
            <button
              onClick={() => {
          ...
          <div className="p-4">
            <div className="mb-4">
              <p className="text-slate-600">
                <strong>Address:</strong> {selectedEvent?.location?.address}, {selectedEvent?.location?.city}
              </p>
              {selectedEvent?.location?.coordinates?.lat && selectedEvent?.location?.coordinates?.lng && (
                <p className="text-slate-500 text-sm mt-1">
                  Coordinates: {selectedEvent.location.coordinates.lat}, {selectedEvent.location.coordinates.lng}
                </p>
              )}
            </div>
            <div 
  ...
  const EventDetailsModal = () => {
    ...
    const hasCoordinates = selectedEvent.location?.coordinates?.lat && selectedEvent.location?.coordinates?.lng;

    const getUserName = (participant) => {
      if (!participant) return "Anonymous User";
      if (participant.user) {
        return participant.user.fullName || participant.user.name || participant.user.fullname || "Anonymous User";
      }
      return participant.fullName || participant.name || participant.fullname || "Anonymous User";
    };
          ...
          <div className="p-6 space-y-6">
                        ...
                        <div key={index} className="flex items-center justify-between p-2 bg-green-50 rounded">
                          <div className="flex items-center gap-2">
                            <div className="w-8 h-8 bg-green-100 rounded-full flex items-center justify-center">
                              <span className="text-green-600 text-sm font-semibold">{getUserInitial(participant)}</span>
                            </div>
                            <div>
                              <p className="text-sm font-medium text-slate-900">{getUserName(participant)}</p>
                        ...
                        <div key={index} className="flex items-center justify-between p-2 bg-yellow-50 rounded">
                          <div className="flex items-center gap-2">
                            <div className="w-8 h-8 bg-yellow-100 rounded-full flex items-center justify-center">
                              <span className="text-yellow-600 text-sm font-semibold">{getUserInitial(participant)}</span>
                            </div>
                            <div>
                              <p className="text-sm font-medium text-slate-900">{getUserName(participant)}</p>
                              <p className="text-xs text-slate-500">
            ...
            {!canViewParts && selectedEvent.participants?.length > 0 && (
              <div className="bg-gray-50 p-4 rounded-lg text-center">
                <p className="text-sm text-gray-600">Participant list is only visible to event organizers and admins.</p>
              </div>
            )}
            ...
            {isLoggedIn ? (
              <div className="flex gap-3 pt-4 border-t">
                {!isJoined && canJoin && (
                  <button onClick={() => handleJoinEvent(selectedEvent._id)} className="btn-primary flex-1">
                    Join Event
                  </button>
                )}
                {isJoined && participantStatus === "confirmed" && (
                  <button onClick={() => handleLeaveEvent(selectedEvent._id)} className="btn-secondary flex-1">
                    Leave Event
                  </button>
                )}
                {isJoined && participantStatus === "waitlist" && (
                  <div className="flex-1 text-center">
                    <Badge variant="warning" className="w-full">On Waitlist</Badge>
                    <button onClick={() => handleLeaveEvent(selectedEvent._id)} className="btn-secondary w-full mt-2">
                      Cancel Waitlist
                    </button>
                  </div>
                )}
                {!isJoined && isFull && (
                  <div className="flex-1 text-center">
                    <Badge variant="warning" className="w-full">Event is Full</Badge>
                    <button onClick={() => handleJoinEvent(selectedEvent._id)} className="btn-secondary w-full mt-2">
                      Join Waitlist
                    </button>
                  </div>
                )}
              ...
              <div className="pt-4 border-t">
                <div className="bg-blue-50 p-4 rounded-lg text-center">
                  <p className="text-sm text-blue-800 mb-2">Please login to join this event</p>
  ...
  // Event Form Modal with Map Integration
      ...
      <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
        <div className="bg-white rounded-lg max-w-2xl w-full max-h-[90vh] overflow-y-auto">
          <div className="p-6">
            ...
            {formError && <FeedbackMessage tone="error" onDismiss={() => setFormError("")}>{formError}</FeedbackMessage>}
            {formSuccess && <FeedbackMessage tone="success" onDismiss={() => setFormSuccess("")}>{formSuccess}</FeedbackMessage>}
            ...
            <form onSubmit={handleSubmitEvent} className="space-y-4">
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                <div>
                  ...
                  <select id="eventType" name="eventType" required className="input" value={formData.eventType} onChange={handleFormChange}>
                ...
                <div>
                  ...
                  <select id="status" name="status" className="input" value={formData.status} onChange={handleFormChange}>
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                <div>
                  ...
                  <input id="startDate" name="startDate" type="datetime-local" required className="input" value={formData.startDate} onChange={handleFormChange} />
                </div>
                <div>
                  <label className="label" htmlFor="endDate">End Date & Time *</label>
                  <input id="endDate" name="endDate" type="datetime-local" required className="input" value={formData.endDate} onChange={handleFormChange} />
                </div>
              </div>
              ...
              <div className="space-y-3">
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                  <div className="flex-1">
                    ...
                    <input id="location.address" name="location.address" type="text" className="input" value={formData.location.address} onChange={handleFormChange} placeholder="Street address" />
                  </div>
                  <div className="flex-1">
                    <label className="label" htmlFor="location.city">City *</label>
                    <input id="location.city" name="location.city" type="text" className="input" value={formData.location.city} onChange={handleFormChange} placeholder="City" />
                  </div>
                </div>
                <button
                  type="button"
                ...
                >
                  Get Coordinates from Address (OpenStreetMap)
                </button>
                {formData.location.coordinates.lat && formData.location.coordinates.lng && (
                  <div className="text-xs text-green-600">
                    ✓ Coordinates set: {formData.location.coordinates.lat}, {formData.location.coordinates.lng}
                  </div>
                )}
              </div>
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                <div>
                  ...
                  <input id="maxParticipants" name="maxParticipants" type="number" min="1" className="input" value={formData.maxParticipants} onChange={handleFormChange} />
                </div>
                <div className="flex items-center">
                  <label className="flex items-center gap-2">
                    <input type="checkbox" checked={formData.reminders} onChange={(e) => setFormData((prev) => ({ ...prev, reminders: e.target.checked }))} className="rounded border-slate-300" />
              ...
              <div>
                ...
                <div className="flex gap-2">
                  <input id="tags" type="text" className="input flex-1" value={tagInput} onChange={(e) => setTagInput(e.target.value)} onKeyPress={(e) => e.key === "Enter" && (e.preventDefault(), handleAddTag())} placeholder="e.g., outdoor, family-friendly" />
                  ...
                  <div className="mt-2 flex flex-wrap gap-2">
                    {formData.tags.map((tag) => (
                      <span key={tag} className="inline-flex items-center gap-1 px-2 py-1 bg-slate-100 rounded text-sm">
                        {tag}
                        <button type="button" onClick={() => handleRemoveTag(tag)} className="text-slate-500 hover:text-red-500">×</button>
                      </span>
                    ))}
                  </div>
                )}
              ...
              <div className="flex gap-3 pt-4 border-t">
                <button type="button" onClick={() => { setShowFormModal(false); resetForm(); }} className="btn-secondary flex-1">Cancel</button>
    ...
    <section className="space-y-6">
      <Card className="border-leaf-100 bg-white/90">
        <SectionHeader
          ...
          right={
            <div className="flex gap-2">
              {isLoggedIn && (
                <button onClick={handleCreateEvent} className="btn-primary text-sm">
                  + Create Event
                </button>
              )}
              <Badge variant="info">Page {pagination.page} of {pagination.pages || 1}</Badge>
            </div>
          }
        />
      </Card>
        ...
        <div className="space-y-4">
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            <div>
              ...
              <input id="city-filter" className="input" placeholder="e.g. Colombo" value={city} onChange={(e) => { setCity(e.target.value); setPage(1); }} />
            </div>
            <div>
              <label className="label" htmlFor="event-type-filter">Event Type</label>
              <select id="event-type-filter" className="input" value={eventType} onChange={(e) => { setEventType(e.target.value); setPage(1); }}>
            ...
            <div>
              ...
              <select id="status-filter" className="input" value={status} onChange={(e) => { setStatus(e.target.value); setPage(1); }}>
                {statusOptions.map((opt) => (<option key={opt.value} value={opt.value}>{opt.label}</option>))}
              </select>
            </div>
            <div className="flex items-end">
              <button className="btn-secondary w-full" onClick={handleResetFilters}>Reset Filters</button>
            </div>
          </div>
          <div className="flex items-center justify-between border-t pt-4">
            <div className="text-sm text-slate-600">Showing {events.length} of {pagination.total} events</div>
            <div className="flex items-center gap-2">
              <button className="btn-secondary" onClick={() => setPage((p) => Math.max(1, p - 1))} disabled={page === 1}>Previous</button>
              <span className="chip">Page {pagination.page} of {pagination.pages || 1}</span>
              <button className="btn-secondary" onClick={() => setPage((p) => Math.min(pagination.pages || p, p + 1))} disabled={page === pagination.pages}>Next</button>
            </div>
          </div>
        </div>
      </Card>
      ...
      {!loading && !error && events.length > 0 && (
        <div className="grid gap-4 lg:grid-cols-2">
          {events.map((event) => {
            ...
            const hasLocation = event.location?.address || event.location?.city;

            return (
              <Card key={event._id} className="cursor-pointer transition hover:-translate-y-0.5 hover:shadow-card" onClick={async () => { await fetchEventDetails(event._id); setShowDetailsModal(true); }}>
                <div className="flex flex-col gap-4">
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      ...
                      <div className="mt-1 flex flex-wrap items-center gap-2">
                        ...
                        {userJoined && (<Badge variant="success" size="sm">Joined</Badge>)}
                        {hasLocation && (<Badge variant="info" size="sm">📍 Has Location</Badge>)}
                      </div>
                    </div>
                    {canManage && (
                      <div className="flex gap-1">
                        <button onClick={(e) => { e.stopPropagation(); handleEditEvent(event); }} className="text-blue-600 hover:text-blue-800 text-sm px-2 py-1 bg-blue-50 rounded">Edit</button>
                        <button onClick={(e) => { e.stopPropagation(); handleDeleteEvent(event._id); }} className="text-red-600 hover:text-red-800 text-sm px-2 py-1 bg-red-50 rounded">Delete</button>
                      </div>
                    )}
                  </div>
                  ...
                  <div className="grid gap-2 text-sm text-slate-600 sm:grid-cols-2">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span>{event.currentParticipants || 0}/{event.maxParticipants || 0}{isFull && <span className="ml-1 text-orange-600">(Full)</span>}</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <span className="font-semibold text-slate-800">👤 Organizer:</span>
                      <span>{event.createdBy?.fullName || event.createdBy?.name || "System"}</span>
                    </div>
                  </div>
                  <div className="text-xs text-leaf-600 hover:text-leaf-700">Click to view details →</div>
                </div>
              </Card>
            );
          })}
      ...
      {!loading && !error && events.length === 0 && (
        <EmptyState
          ...
          description={city || eventType || status ? "Try adjusting your filters to see more events." : "No events are currently scheduled. Check back later for community activities!"}
